...
- **Profiling:** `MAC_OCR_PROFILE=1` prints per-page OCR timing plus a run total to stderr (full-page vs partitioned-pass seconds, partition count, render/write seconds, observation counts). Use it to find why a run is slow — cost is dominated by serial Vision passes multiplied by partitioning. stderr only; works with `-o -`.
