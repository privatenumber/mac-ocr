...
- **`--ocr-strategy auto|standard|partitioned`** controls searchable-PDF OCR strategy. `auto` is default and may run recursive partitioned OCR for large pages with small detected text; `standard` forces full-page OCR only; `partitioned` forces partitioned OCR for eligible pages. Pair with `MAC_OCR_DEBUG=1` and inspect `recognition.passes.partitioned`, `origin`, and `rejection.reason`/`supersededBy` in the sidecar. Auto skips partitioning when `--roi` is set; forced `partitioned` mode rejects `--roi`.
