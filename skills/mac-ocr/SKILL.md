...
- **`MAC_OCR_TILED=1`** enables an experimental extra tiled OCR pass for searchable PDFs. It can recover small text missed by full-page Vision OCR, but can add duplicate/partial text; pair it with `MAC_OCR_DEBUG=1` and inspect `sourcePass`, `tile`, and `edgeTouching` in the sidecar.
