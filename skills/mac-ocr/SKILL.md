...
The package also exposes a typed, promise-based API (`import { ocr, createSearchablePdf, supportedLanguages } from 'mac-ocr'`) backed by the bundled binary. Inputs are **bytes** (Buffer/Uint8Array/ArrayBuffer) — read files or fetch URLs in your own code. Ordinary main-thread `ocr()` calls reuse one hidden native service per Node process, including calls canceled with an AbortSignal; worker-thread, streaming, and artifact calls retain one-shot subprocesses.
