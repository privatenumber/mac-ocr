...
To see where time goes, set `MAC_OCR_PROFILE=1`. It prints a per-page timing line plus a run total to stderr — full-page OCR seconds, partitioned-pass seconds and partition count, raster and write seconds, and accepted/rejected observation counts — so the dominant cost (serial Vision passes, multiplied by partitioning) is visible without Instruments. Profiling writes to stderr only, never touches the PDF, and works with `-o -`.
