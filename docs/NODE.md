...
Throws a `MacOcrError` (`kind: 'usage'`) if `input` is a multi-page PDF. Throws `kind: 'unavailable'`, `code: 'document_recognition_unavailable'` on macOS before 26.

## `ocrDocument.pages(input, options?)`

Recognizes every page of a PDF as structured document content. It has the same lazy, single-use streaming semantics as `ocr.pages`:
...
| Option | Type | Effect |
|---|---|---|
| `languages` | `string[]` | Document-recognition language identifiers, such as `['en']`. These are two-letter ISO identifiers; ordinary OCR's regional BCP-47 identifiers such as `en-US` are not accepted. |
...
| `signal` | `AbortSignal` | Abort this one-shot subprocess |
...
type DocumentTableCell = {
  rowRange: { start: number; end: number } // inclusive, zero-based
  columnRange: { start: number; end: number } // inclusive, zero-based
  content: DocumentContainer
}
...
All document geometry is normalized with a top-left origin. `DocumentContainer` is recursive because table cells and list items can contain paragraphs, tables, lists, and text. Root `text` is the non-duplicated convenience transcript; structural views are not meant to be concatenated.
...
Worker-thread calls, `ocr.pages()`, `ocrDocument()`, `ocrDocument.pages()`, `createSearchablePdf()`, and `supportedLanguages()` run as one-shot processes. Abort and await active worker calls before forcibly terminating a worker, because termination can skip JavaScript cleanup while the one-shot child finishes.
